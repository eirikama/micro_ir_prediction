...
cdef(double, double, double) q_ext_sca_na_scalar(
    ...
    cdef int nsca = pi.shape[0]
    nmax = min(nmax, pi.shape[1])

    cdef double * bx = <double * >malloc(sizeof(double) * (nmax + 1))
    cdef double complex * bz = <double complex * >malloc(sizeof(double complex) * (nmax + 1))
    cdef double[3] yx
    ...
    for mult in range(2 * (nmax + 50 - 2) + 3, 2 * (nmax - 2) + 3, -2):
        bx[nmax - 1], bx[nmax] = mult * bx[nmax - 1] * \
            ONE_OVER_X - bx[nmax], bx[nmax - 1]
        bz[nmax - 1], bz[nmax] = mult * bz[nmax - 1] * \
            ONE_OVER_Z - bz[nmax], bz[nmax - 1]
    ...
    cdef double complex m2 = m ** 2
    cdef double ax
    cdef double complex az, ahx, an, bn
    cdef int cn
    cdef double qext = 0
    cdef double qsca_total = 0

    cdef double complex * s1 = <double complex * >malloc(sizeof(double complex) * nsca)
    cdef double complex * s2 = <double complex * >malloc(sizeof(double complex) * nsca)
    ...
    qsca_total += cn * ((an * an.conjugate()).real +
                        (bn * bn.conjugate()).real)
    for i in range(nsca):
        s1[i] = cn * 0.5 * (an * pi[i, 0] + bn * tao[i, 0])
        s2[i] = cn * 0.5 * (bn * pi[i, 0] + an * tao[i, 0])
    hx[0] = hx[1]
    ...
    for n in range(2, nmax):
        ...
        for i in range(nsca):
            s1[i] += cn / (n * (n + 1.)) * \
                (an * pi[i, n - 1] + bn * tao[i, n - 1])
            s2[i] += cn / (n * (n + 1.)) * \
                (bn * pi[i, n - 1] + an * tao[i, n - 1])

    # trapezoid integration over NA
    cdef double qsca_na = 0
    cdef double q1, q2
    for i in range(1, nsca - 1):
        q1 = (s1[i] * s1[i].conjugate()).real + \
            (s2[i] * s2[i].conjugate()).real
        qsca_na += q1 * sin_thetas[i]
    q1 = (s1[0] * s1[0].conjugate() + s2[0] * s2[0].conjugate()).real
    q2 = (s1[nsca - 1] * s1[nsca - 1].conjugate() +
          s2[nsca - 1] * s2[nsca - 1].conjugate()).real
...
def q_ext_sca_na(
    ...
    Parameters
    ----------
    ms     : refractive index (float or array-like, complex)
    lambds : wavelengths in same units as rs
    rs     : particle radii in same units as lambds
    theta_na         : NA half-angle in radians (default 0.4)
    theta_resolution : number of angular quadrature points (default 12)
    theta_min        : minimum angle (default 0.0)

    Returns
    -------
    qext, qsca, qsca_na : np.ndarray
    """
