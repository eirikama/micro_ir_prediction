...
from scipy.special.cython_special cimport jv, yv
...
cdef (double, double, double) q_ext_sca_na_scalar(
    ...
    qsca_total  += cn * ((an * an.conjugate()).real + (bn * bn.conjugate()).real)
    for i in range(nsca):
    ...
    for n in range(2, nmax):
        ...
        for i in range(nsca):
            s1[i] += cn / (n*(n+1.)) * (an * pi[i, n-1] + bn * tao[i, n-1])
            s2[i] += cn / (n*(n+1.)) * (bn * pi[i, n-1] + an * tao[i, n-1])

    # trapezoid integration over NA
    cdef double qsca_na = 0
    cdef double q1, q2
    for i in range(1, nsca - 1):
        q1 = (s1[i] * s1[i].conjugate()).real + (s2[i] * s2[i].conjugate()).real
        qsca_na += q1 * sin_thetas[i]
    q1 = (s1[0] * s1[0].conjugate() + s2[0] * s2[0].conjugate()).real
    q2 = (s1[nsca-1] * s1[nsca-1].conjugate() + s2[nsca-1] * s2[nsca-1].conjugate()).real
    ...
    free(s1);   free(s2);   free(bx);   free(bz)
