...
class SpectralDataset(IterableDataset):
    ...
    def __iter__(self):
        while True:
            ...
            if self.remainder > 0:
                extra_indices = np.random.choice(np.arange(len(self.y)), self.remainder, replace=True)
    ...
    def _process_batch(self, idx):
        ...
        if mie_mask.any():
            ...
            n0s, rs, n_ims, hs, scs = [np.random.uniform(l, h, (s_subset.shape[0], 1)) 
                                       for l, h in [(1.25, 1.65), (2., 14), (1e-4, 1e-2), (1.5, 2.5), (1.5, 2.5)]]
        ...
        for mask, ranges in zip([poly_mask, bkg_mask], 
...
class SpectralDataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        l, s, wn, label_encoding = get_training_data(self.split, self.config.zarr_path, self.config.spectra_per_plastic * 2, patch_size=32)
        self.wn = wn
