...
def run_inference_pipeline(
    ...
    # Under `--multirun` Hydra's basic launcher runs every job in the SAME
    # process, so the model, Trainer and cached allocator blocks from the
    # training run that just finished are still resident. Lightning's
    # Trainer/model/callback reference cycles only die on a gc pass, so
    # reclaim explicitly before loading the inference checkpoint — a late
    # sweep job failing to load a small checkpoint (while the same config
    # succeeds standalone) is the signature of that accumulation.
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        log.info(
            "[inference] GPU memory before load: %.2f GB allocated / %.2f GB reserved",
            torch.cuda.memory_allocated() / 1e9,
            torch.cuda.memory_reserved() / 1e9,
        )

    # The `with` below is wrapped so a failure opening the LMDB store or
    # loading the checkpoint is logged here. Without it the traceback reaches
    # only Hydra's per-job log file and the console shows nothing at all
    # between "Starting inference" and the next sweep job. Per-image errors
    # are handled inside the loop, so this only fires on setup failures.
    # NOTE: open_pred_store is a @contextmanager — the LMDB is opened by
    # __enter__, not by the call, so the try must span the `with` itself.
    try:
        with open_pred_store(cfg.inference.pred_store_path) as pred_store, \
             InferenceSession(
                 ...
                 # Same preprocessing as training, with the state fitted on
                 # the training split (loaded from the sidecar beside the ckpt).
        ...
        log.exception(
            "Inference setup failed before any image was processed "
            "(pred_store=%s, ckpt=%s, devices=%s)",
            cfg.inference.pred_store_path,
            cfg.inference.ckpt_path,
            list(cfg.inference.devices),
        )
        raise
