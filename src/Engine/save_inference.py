...
def save_inference_outputs_zarr(
    ...
    img_grp.array("argmax_map", np.argmax(prob_map, axis=-1).astype(np.uint8),
                  dtype="u1", overwrite=True,
                  chunks=chunk_hw, compressor=compressor)

    img_grp.array("bg_prob", prob_map[:, :, background_idx].astype(np.float16),
    ...
    img_grp.array("top_k_classes",
                  ...
                  dtype="u1", overwrite=True,
                  chunks=chunk_hwk, compressor=compressor)
