    ...
    with mlflow.start_run(run_name=run_name) as run:
        try:
            ...
            if cfg.mode in ["train", "all"]:
                ...
                best_path, best_score, final_epoch, stopped_early = run_training(cfg, model, datamodule, mlf_logger)

                train_sec = time.time() - t0
                mlflow.log_metric("Timer/train_duration_min", train_sec / 60)

                mlflow.log_metrics({"best_val_score": best_score, "final_epoch":   final_epoch,})
                ...
                with open_pred_store(cfg.pred_store_path) as pred_store:
                    for i, img_data in enumerate(test_images):
                        ...
                        try:
                            ...
                            mlflow.log_metric(f"Timer/infer_min_{safe_label}", (time.time() - t0)/60)
                ...
                if image_metrics:
                    ...
                    if accs:
                        ...
                        for cls, cls_accs in class_accs.items():
                            ...
                            mlflow.log_metric(f"Inference/mean_acc_{safe}", float(np.mean(cls_accs)))
